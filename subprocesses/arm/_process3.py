...
# Control mode state
control_mode = "ik"              # current control mode
last_toggle_input = 0            # for rising-edge detection
toggle_control_mode_input = 0    # current toggle input
...
# -------------------------
# TOGGLE CONTROL MODE CALLBACK
# -------------------------
async def handle_toggle_mode(value: float):
    global toggle_control_mode_input
    toggle_control_mode_input = value
...
async def telemetry_loop(control_socket: ControlSocket, interval: float):
    global last_time, last_toggle_input, control_mode
    ...
    while True:
        ...
        # ---- Handle mode toggle (rising edge) ----
        if last_toggle_input == 0 and toggle_control_mode_input != 0:
            control_mode = "man" if control_mode == "ik" else "ik"
            logger.info(f"Switched control mode to: {control_mode}")
        last_toggle_input = toggle_control_mode_input

        # integrate velocities (mm/s and deg/s)
        for key in desired_position.keys():
            desired_position[key] += velocity_inputs[key] * dt

        success = True  # default for telemetry
        ...
        # ---- Manual mode (currently unimplemented) ----
        else:
            # Could implement manual control here later
            pass
        ...
        await control_socket.outputs.update_output("control_mode", control_mode)
...
async def main(
    ...
    # Register toggle control mode input
    schema.register_axis(
        control_socket.inputs,
        "toggle_control_mode",
        callback=lambda v: asyncio.create_task(handle_toggle_mode(v)),
    )
    ...
    control_socket.outputs.register_output("control_mode")
